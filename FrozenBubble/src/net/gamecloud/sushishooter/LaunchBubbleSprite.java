...
{ 
 ...
 // private static int isBubbleOrAxe = 0;
 // public static boolean isBubble = false;
  static int nextBubble = 0;
  static int bubblesShotCounter = 0;
                            ...
                            BmpWrap[] bubbles, BmpWrap[] colorblindBubbles)
  {
   ...
   // LaunchBubbleSprite.bubblesShotCounter++;
   // if (LaunchBubbleSprite.bubblesShotCounter % 2 == 0)
    //	LaunchBubbleSprite.nextBubble = 0;
   // else if (LaunchBubbleSprite.bubblesShotCounter % 2 != 0)
    //	LaunchBubbleSprite.nextBubble = 2;
   // LaunchBubbleSprite.isBubbleOrAxe++;
    //System.out.println(isBubbleOrAxe);
  }
  ...
  { 
	//LaunchBubbleSprite.isBubbleOrAxe++;
	//System.out.println(isBubbleOrAxe);
	// is axe or bubble checked
    if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL) {
    	//if (LaunchBubbleSprite.isBubbleOrAxe % 2 == 0) {
    		drawImage(bubbles[currentColor], 302, 390, c, scale, dx, dy);
    		//LaunchBubbleSprite.isBubble = true;
    	//}
    	//else if (LaunchBubbleSprite.isBubbleOrAxe % 2 != 0) {
    		//drawImage(bubbles[currentColor], 302, 390, c, scale, dx, dy);
    		//LaunchBubbleSprite.isBubble = false;
    	//}
    } else {
