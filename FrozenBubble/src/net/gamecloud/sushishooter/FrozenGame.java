...
public class FrozenGame extends GameScreen {
	...
	// Vector to store all bubbles
	public static Vector<BubbleSprite> allBubbles = new Vector<BubbleSprite>();
	
	static int bubbleShootCounter = 0;
	static int typeOfBubble = 0;
  ...
  public BmpWrap[] getBubbleImages() {
	  return bubbles;
  }
  ...
  static BubbleSprite[][] bubblePlay;
                    ...
                    LevelManager levelManager_arg)
  {
    ...
    bubblePlay = new BubbleSprite[14][14];
      ...
      for (int i=j%2 ; i< 6 ; i++) {
        if (currentLevel[i][j] != -1) {
          BubbleSprite newOne = new BubbleSprite(
               ...
               currentLevel[i][j],
               bubbles[2], bubblesBlind[currentLevel[i][j]],
               frozenBubbles[currentLevel[i][j]], bubbleBlink, bubbleManager,
               soundManager, this);
     //     newOne.typeOfBubble = BubbleSprite.TREE;
     //     newOne.frozenify();
          bubblePlay[i][j] = newOne;
         // bubblePlay[i][j].typeOfBubble = BubbleSprite.TREE;
         // bubblePlay[i][j].frozenify();
          this.addSprite(newOne);
          // new bubble to vector
          allBubbles.add(newOne);
    ...
    currentColor = 0; //bubbleManager.nextBubbleIndex(random);
    nextColor = 2; //bubbleManager.nextBubbleIndex(random);
    //FrozenGame.bubbleShootCounter++;
    if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL) {
      if (FrozenGame.bubbleShootCounter % 4 != 0)
   ...
   // this.addSprite(nextBubble);
    ...
    launchBubble = new LaunchBubbleSprite(0, 
                      ...
                      boolean ats_touch_fire, double ats_touch_dx)
  {
	...
	//System.out.println(FrozenGame.bubbleShootCounter);
	// vaihda kuplan tyyppia
	if (FrozenGame.bubbleShootCounter % 4 != 0)
		FrozenGame.typeOfBubble = 2;
	else if (FrozenGame.bubbleShootCounter % 4 == 0)
		FrozenGame.typeOfBubble = 0;
	//System.out.println(typeOfBubble + " on bubbleType");
    boolean ats = FrozenBubble.getAimThenShoot();
    if ((ats && ats_touch_fire) || (!ats && touch_fire)) {
      key_fire = true;
      FrozenGame.bubbleShootCounter++;//paskaa
    }
    ...
    if (endOfGame) {
      ...
      if (move[FIRE] == KEY_UP || hurryTime > 480) {
        if (movingBubble == null && readyToFire) {
          ...
          //nextColor = FrozenGame.typeOfBubble;
      /*    int seuraava = 0;
          if (FrozenGame.bubbleShootCounter % 4 == 0)
        	  seuraava = 2;
          else if (FrozenGame.bubbleShootCounter % 4 != 0)
        	  seuraava = 0;
          if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL) {
            nextBubble.changeImage(bubbles[seuraava]);
          } else {
            nextBubble.changeImage(bubblesBlind[nextColor]);
          }
          */
          launchBubble.changeColor(FrozenGame.typeOfBubble);
